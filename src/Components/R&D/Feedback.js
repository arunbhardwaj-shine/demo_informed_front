                    ...
                    feedbackData?.map((item, index) => (
                      <>
                        <div className="wrap_div">
                          {
                            activeTab?.current == 1 ?
                            <div className= {item.reply_status ? "radio_btn div_checked" : "radio_btn"}>
                                  <input 
                                    type="checkbox"
                                    value={item._id}
                                    name={item._id}
                                    checked={item.reply_status}
                                    onChange={(e) => handleOptionChange(e,item._id,'feedback')}
                                  />
                              ...
                              <div className= {item.reply_status ? "radio_btn div_checked" : "radio_btn"}>
                                  <input 
                                    ...
                                    name={item._id}
                                    checked={item.reply_status}
                                    onChange={(e) => handleOptionChange(e,item._id,'question')}
                                  />
                                  <span class="checkmark"></span>
                                 
                              </div>
                          }
                            <div className={activeTab?.current == 1 ? "timeline-block feedback" : "timeline-block"} key={index}>
                              <div className="timeline-block-head library">
                                <div className="timeline-block-title d-flex flex-column align-items-start">
                                  ...
                                  {
                                    activeTab?.current == 2 ?
                                    <h6><span>Article Title:</span> {item?.title ? item?.title : "N/A"}</h6>
                                    : null
                                  }

                                  <h6><span>
                                    {
                                      activeTab?.current == 1 ? <span>Feedback:</span> : <span>Question:</span>
                                    }
                                    </span>
