 ...
 const isLikeRdAccount= rdLikeArray.includes(localStorage.getItem("user_id"))
        ...
        if (res.data.status_code == 200) {
          if (flag == "update") {
            popup_alert({
              ...
              type: "success",
              redirect: "/webinar/email/smartlist",
            });
          } else {
            var path = "";
            var x = localStorage.getItem("webinar_sd_i");
  ...
  const addMoreHcp = () => {
    ...
    if (status.every((element) => element == "true")) {
      ...
      if (isLikeRdAccount || localStorageUserId == "m5JI5zEDY3xHFTZBnSGQZg==") {
    ...
    if (activeManual == "active") {
      ...
      const status = body.data.map((data, index) => {
        if (
          ...
          return;
        } else if (data.institution_type == "" && isLikeRdAccount) {
          setValidationError({
            institutionType: "Please select the Institution type",
            index: index,
          });
          return;
        }else if (data.investigator_type == "" && isLikeRdAccount) {
          setValidationError({
              ...
              <div className="col-12 col-md-1">
                <div className="header-btn-left">
                  <button className="btn btn-primary btn-bordered back">
                    <Link to={"/webinar/email/smartlist/createsmartlist"}>Back</Link>
                  </button>
                </div>
              </div>
              ...
              <div className="col-12 col-md-3">
                <div className="header-btn">
                  <button className="btn btn-primary btn-bordered move-draft">
                    <Link to={{ pathname: "/webinar/email/smartlist/createsmartlist" }}>Cancel</Link>
                  </button>
                  <button
