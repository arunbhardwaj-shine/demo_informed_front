  ...
  const changeStatus = async (status, question_id, e, current_state) => {
    try {
      ...
      if (current_state === "new" && status === 2) {
        let question = questions?.[current_state].find(
      ...
      const response = await postData(
        ENDPOINT.WEBINAR_CHANGE_QUESTION_STATUS,
        body
      );
      const sourceIndex = questions?.[current_state].findIndex(
        (item) => item.id === question_id
      );
      if (sourceIndex !== -1) {
        const updatedQuestions = { ...questions };
        const removedItem = updatedQuestions?.[current_state].splice(
          sourceIndex,
          1
        )[0];
        if (status === 0) {
  ...
  const deleteRegisterUser = async (id) => {
    ...
    try {
      ...
      const res = await deleteMethod(
        `${ENDPOINT.WEBINAR_DELETE_USER}/${id}/${event}`
      );
  ...
  const handleSendMessage = async () => {
    try {
      if (adminMessage.trim() === "") {
        ...
        const response = await postData(
          ENDPOINT.WEBINAR_SEND_ADMIN_QUESTION,
          body
        );
        setAdminMessage("");
        setErrorMessage("");
  ...
  const toggleAccordion = async (id, user_id) => {
    try {
      ...
      if (accordionOpen !== id) {
        ...
        const response = await postData(
          ENDPOINT.WEBINAR_GET_EVENT_ATTENDEES_DETAILS,
          body
        );
  ...
  const downloadExcelUsers = (data) => {
    try {
      ...
      data = data?.map((item, index) => {
        ...
        finalData.Speed = extraDetails?.speed+"Mb" || "0.78MB";
        }
              ...
              <div className="doc-content-main-box col">
                <div className="live-stream-tabs-data">
                  <Tabs activeKey={activeTab} onSelect={handleTabSelect} fill>
                    <Tab eventKey="new" title="New " className="flex-column">
                                  ...
                                  <>
                                    ...
                                    >
                                      ...
                                      <div className="live-stream-action d-flex justify-content-end align-items-center">
                                        <Button
                                          className="send-speaker"
                                          onClick={(e) =>
                                            changeStatus(2, item.id, e, "new")
                                          }
                                        >
                                          Send To Speaker
                                        </Button>
                                        <div className="clear-search">
                                          <button
                                            onClick={(e) =>
                                              showConfirmationPopup(e, item?.id)
                                            }
                                          >
                                            <svg
                                              width="24"
                ...
                <div className="dm-speaker-txt">
                  ...
                  >
                    ...
                    <Button
                      ...
                      disabled={messageSendStatus ? "disabled" : false}
                    >
                      Send
                    </Button>
                  </div>
                        ...
                        <div className="clear-search d-flex align-items-center">
                          ...
                          {attendees.length > 0 ? (
                            ...
                            <button
                              className={
                                deleteStatus
                                  ? "btn btn-outline-primary active delt_btn"
                            ...
                            >
                              {
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                  <rect width="4.08952" height="29.5976" rx="2.04476" transform="matrix(0.70183 0.712345 -0.70183 0.712345 21.125 0.00195312)" fill="#0066be"/>
                                  <rect width="4.08952" height="29.5976" rx="2.04476" transform="matrix(0.70183 -0.712345 0.70183 0.712345 0 2.91406)" fill="#0066be"/>
                                </svg>
                                :
                                <svg
                                  width="24"
                          ...
                          <div className="btn-refresh">
                            <img
                              className={
                                refreshAttendeesFlag === "online"
                                  ? "refresh-rotate"
                                  : ""
                              }
                              src={path_image + "refresh-btn.svg"}
                              alt=""
                              onClick={(e) => refreshAttendees("online")}
                            />
                          </div>
                        </div>
                      </div>
                                        ...
                                        <div className="live-stream-hcp d-flex align-items-center justify-content-between">
                                          ...
                                          {deleteStatus ? (
                                            <div className="clear-search">
                                              <button
                                                onClick={(e) =>
                                                  showAttendeesConfirmationPopup(
                                                    e,
                                                    item?.userId
                                                  )
                                                }
                                        ...
                                        {item?.id === accordionOpen ? (
                                          moreDetailsStatus ? (
                                            ...
                                            <div className="details-content">
                                              <ul>
                                                {Object.keys(attendeesDetails)
                                                  ?.length > 0 ? (
                                                  <ul>
                                                    <li>
                                                      <span>Login Time</span>
                                                      {
                                                        attendeesDetails?.login_time
                                                      }{" "}
                                                      |{" "}
                                                      {
                                                    ...
                                                    <li>
                                                      ...
                                                      {attendeesDetails?.speed}
                                                    </li>
                                                    <li>
                                                      <span>Browser </span>
                                                      {
                                                        attendeesDetails?.browser
                                                      }
                                                    </li>
                                                    <li>
                                                      <span>Device</span>
                                                      {attendeesDetails?.device}
                                                    </li>
                                                  </ul>
                                                ) : (
                      ...
                      <div className="doc-content d-flex justify-content-between align-items-center">
                        ...
                        <button
                          ...
                          <div className="btn-refresh">
                            <img
                              className={
                                refreshAttendeesFlag === "left"
                                  ? "refresh-rotate"
                                  : ""
                              }
                              src={path_image + "refresh-btn.svg"}
                              alt=""
                              onClick={(e) => refreshAttendees("left")}
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="doc-content d-flex justify-content-between align-items-center">
                        ...
                        <button
                          ...
                          <div className="btn-refresh">
                            <img
                              className={
                                refreshAttendeesFlag === "question_ask"
                                  ? "refresh-rotate"
                                  : ""
                              }
                              src={path_image + "refresh-btn.svg"}
                              alt=""
                              onClick={(e) => refreshAttendees("question_ask")}
                            />
                          </div>
                        </div>
                      </div>
                                          ...
                                          <div className="hcp-activity-status">
                                            <div className="hcp-activity-links">
                                              ...
                                              >
                                                ...
                                                >
                                                  <path
                                                    d="M23.92 5.28516L12.8457 11.868C12.5899 12.0144 12.3004 12.0913 12.0057 12.0913C11.711 12.0913 11.4215 12.0144 11.1657 11.868L0.0799999 5.28516C0.0270091 5.51376 0.000170336 5.74764 0 5.9823V17.1594C0 17.9505 0.314264 18.7092 0.873659 19.2686C1.43305 19.828 2.19175 20.1423 2.98286 20.1423H21.0171C21.8082 20.1423 22.5669 19.828 23.1263 19.2686C23.6857 18.7092 24 17.9505 24 17.1594V5.9823C23.9998 5.74764 23.973 5.51376 23.92 5.28516Z"
                                                    fill="#0066BE"
                                                  ></path>
                                                  <path
                                                    d="M12.2745 10.92L23.4517 4.26857C23.1772 3.87765 22.8128 3.55839 22.3891 3.33763C21.9655 3.11687 21.4951 3.00108 21.0174 3H2.98311C2.50543 3.00108 2.03499 3.11687 1.61138 3.33763C1.18776 3.55839 0.823359 3.87765 0.548828 4.26857L11.7374 10.92C11.8198 10.965 11.9121 10.9886 12.006 10.9886C12.0998 10.9886 12.1922 10.965 12.2745 10.92Z"
                                                    fill="#0066BE"
                                                  ></path>
                                                </svg>
                                              </button>
