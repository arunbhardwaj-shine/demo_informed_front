import React from 'react'
import { Col, Row } from 'react-bootstrap'
import Dropdown from 'react-bootstrap/Dropdown';

const DefaultWebinar = () => {

  const path = process.env.REACT_APP_ASSETS_PATH_INFORMED_DESIGN

  return (
   <>
      <Col className="col right-sidebar">
        <div className="custom-container">
            <Row>
             <div className = "container" style = {{display:"flex",flexDirection:"row"}}>
  

   

   

 


            <div className="table_xls search_view">
                <div className="smart-list-btns">
                  <div className="top-right-action">
                    <div className="search-bar">
                      <form
                        className="d-flex"
                       // onSubmit={(e) => submitHandler(e)}
                      >
                        <input
                          className="form-control me-2"
                          type="search"
                          placeholder="Search"
                          aria-label="Search"
                         // onChange={(e) => searchChange(e)}
                        />
                      
                          <button
                            className="btn btn-outline-success"
                            type="submit"
                          >
                            <svg
                              width="16"
                              height="16"
                              viewBox="0 0 16 16"
                              fill="none"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <path
                                d="M15.8045 14.862L11.2545 10.312C12.1359 9.22334 12.6665 7.84 12.6665 6.33334C12.6665 2.84134 9.82522 0 6.33325 0C2.84128 0 0 2.84131 0 6.33331C0 9.82531 2.84132 12.6667 6.33328 12.6667C7.83992 12.6667 9.22325 12.136 10.3119 11.2547L14.8619 15.8047C14.9919 15.9347 15.1625 16 15.3332 16C15.5039 16 15.6745 15.9347 15.8045 15.8047C16.0652 15.544 16.0652 15.1227 15.8045 14.862ZM6.33328 11.3333C3.57597 11.3333 1.33333 9.09066 1.33333 6.33331C1.33333 3.57597 3.57597 1.33331 6.33328 1.33331C9.0906 1.33331 11.3332 3.57597 11.3332 6.33331C11.3332 9.09066 9.09057 11.3333 6.33328 11.3333Z"
                                fill="#97B6CF"
                              ></path>
                            </svg>
                          </button>
                
                      </form>
                    </div>
                  </div>
                </div>
              </div>
                            
              </div> 
               
   

   

   


    
            </Row>
           <div className="selected-hcp-list">
            <table className="table" id="table-to-xls">
              <thead className="sticky-header">
                <tr>
                  <th scope="col">Select Event      
                  
                  <Dropdown>
      
      <Dropdown.Toggle variant="light" id="dropdown-basic">
      Select Event
      </Dropdown.Toggle>

      <Dropdown.Menu>
        <Dropdown.Item href="#/action-1">Action</Dropdown.Item>
        <Dropdown.Item href="#/action-2">Another action</Dropdown.Item>
        <Dropdown.Item href="#/action-3">Something else</Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown></th>
                 <div>
                     
                  </div> 
      
                  <th scope="col">Select Country            <Dropdown>
      
      <Dropdown.Toggle variant="light" id="dropdown-basic">
      Select Event
      </Dropdown.Toggle>

      <Dropdown.Menu>
        <Dropdown.Item href="#/action-1">Action</Dropdown.Item>
        <Dropdown.Item href="#/action-2">Another action</Dropdown.Item>
        <Dropdown.Item href="#/action-3">Something else</Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown></th>
                  <th scope="col">Select User type            <Dropdown>
      
      <Dropdown.Toggle variant="light" id="dropdown-basic">
      Select Event
      </Dropdown.Toggle>

      <Dropdown.Menu>
        <Dropdown.Item href="#/action-1">Select Type</Dropdown.Item>
        <Dropdown.Item href="#/action-2">Another action</Dropdown.Item>
        <Dropdown.Item href="#/action-3">Something else</Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown></th>
                  <th scope="col">Country            <Dropdown>
      
      <Dropdown.Toggle variant="light" id="dropdown-basic">
      Select Event
      </Dropdown.Toggle>

      <Dropdown.Menu>
        <Dropdown.Item href="#/action-1">Action</Dropdown.Item>
        <Dropdown.Item href="#/action-2">Another action</Dropdown.Item>
        <Dropdown.Item href="#/action-3">Something else</Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown></th>
                  <th scope="col">Business Unit            <Dropdown>
      
      <Dropdown.Toggle variant="light" id="dropdown-basic">
      Select Event
      </Dropdown.Toggle>

      <Dropdown.Menu>
        <Dropdown.Item href="#/action-1">Action</Dropdown.Item>
        <Dropdown.Item href="#/action-2">Another action</Dropdown.Item>
        <Dropdown.Item href="#/action-3">Something else</Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown></th>
                  <th scope="col">Contact Type            <Dropdown>
      
      <Dropdown.Toggle variant="light" id="dropdown-basic">
      Select Event
      </Dropdown.Toggle>

      <Dropdown.Menu>
        <Dropdown.Item href="#/action-1">Action</Dropdown.Item>
        <Dropdown.Item href="#/action-2">Another action</Dropdown.Item>
        <Dropdown.Item href="#/action-3">Something else</Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown></th>
            
                </tr>
              </thead>
              <tbody className="form-group">
                <tr className="seprator-add">
                  <td colspan="5"></td>
                </tr>
                <tr id={`row-selected`}>
                    <td id={`field_name` }>
                      <span> {"Anuj"+ " " + "Sharma"} </span>
                    </td>

                    <td id={`field_email`}>{"risindezires@gmail.com"}</td>
                  
                    <td id={`field_bounced` }>0</td>
                 
                    <td id="field_business_unit">ibu</td>
                    <td id="field_interest">
                        interest
                    </td>
                  </tr>

                   <tr  id={`row-selected`}>
                    <td id={`field_name` }>
                      <span> {"Anuj"+ " " + "Sharma"} </span>
                    </td>
                    <td id={`field_email`}>{"risindezires@gmail.com"}</td>
                    <td id={`field_bounced` }>0</td>
                    <td id="field_business_unit">ibu</td>
                    <td id="field_interest">interest</td>
                  </tr>
                  <tr id={`row-selected`}>
                    <td id={`field_name` }>
                      <span> {"Anuj"+ " " + "Sharma"} </span>
                    </td>
                    <td id={`field_email`}>{"risindezires@gmail.com"}</td>
                    <td id={`field_bounced` }>0</td>
                    <td id="field_business_unit">ibu</td>
                    <td id="field_interest">interest</td>
                  </tr>


                   <tr
                    id={`row-selected`}
              
                  >
                    <td
                      id={`field_name` }
                    
                    >
                      <span> {"Anuj"+ " " + "Sharma"} </span>
                    </td>

                    <td id={`field_email`}>{"risindezires@gmail.com"}</td>
                  
                    <td id={`field_bounced` }>0</td>
                 
                    <td id="field_business_unit">ibu</td>
                    <td id="field_interest">
                        interest
                    </td>




                

                  </tr>
                  

                  
               
              
              </tbody>
            </table>
          </div>
        </div>
    </Col>
    </>
  )
}

export default DefaultWebinar