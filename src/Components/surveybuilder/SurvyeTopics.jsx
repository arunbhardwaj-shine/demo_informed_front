...
function SurveyTopics() {
  ...
  const [originalTopicData, setOriginaltopicData] = useState({});
  ...
  const initFun = async () => {
    try {
      ...
      setOriginaltopicData(resp?.data?.data)
    } catch (err) {
      console.log("--err", err);
    } finally {
      loader("hide");
  ...
  const handleConfirmModel = async (id) => {
    ...
    try {
      ...
      popup_alert({
        visible: "show",
        message: "Your topic has been deleted <br />successfully !",
        type: "success",
        redirect: "",
      });
    } catch (err) {
  ...
  const setCommonModel = (stateMsg, e, id, value) => {
    ...
    if (stateMsg == "Update") {
      ...
      setTopicId(id);
      setHeading(`Update ${content?.label?.trim()}`);
...
const submitHandler = (e) => {
    ...
    if (trimmedSearch.length > 0) {
      const filteredData = originalTopicData.data.filter((item) =>
        item.product?.trim().toLowerCase().includes(trimmedSearch)
      );
      setProductData({ data: filteredData, flag: false });
    } 
  };
