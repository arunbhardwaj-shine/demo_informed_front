...
const ContentGraph = () => {
  ...
  const getDataContentGraph = async () => {
    try {
      loader("show");
      const response = await postData(ENDPOINT.CONTENT_TYPE_GRAPH);
  ...
  const getDataFromApi = async (type = "all") => {
    ...
    try {
      ...
      console.log("Tab", activeTab.current);
      if (hadData.length <= 0) {
          ...
          <Row>
            <div className="delivery-trends">
              <div className="custom-container">
                ...
                {data?.length ? (
                  <div className="high_charts mt-4">
