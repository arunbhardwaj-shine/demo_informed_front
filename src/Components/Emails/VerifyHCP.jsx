...
const VerifyHCP = (props) => {
  ...
  const updateReader = (readers_d = "", type = 1) => {
    if (type == 1) {
      var reducHcp = readers_d?.map((item) => {
    ...
    axios
      ...
      .then((res) => {
        const reeaderData = res?.data?.response?.data ? res?.data?.response?.data : [];
        setSelectedHcp(reeaderData);
        loader("hide");
      })
      .catch((err) => {
        loader("hide");
  ...
  const nextClicked = () => {
    ...
    if(irtRoleObj?.IRTFlag){
      ...
      const nextRoute = routeTypeSurvey ? '/survey/email/verify-hcp-mail' : '/VerifyHcpMAIL';
      props.getSelected(selectedHcp);
      props.getSearched(searchedUsers);
      navigate(nextRoute, {
        state: {
  ...
  const saveAsDraft = async () => {
    ...
    const body = {
      ...
      route_location: routeTypeSurvey ? "survey/email/verify-hcp" : "VerifyHCP",
