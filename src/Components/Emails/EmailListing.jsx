...
const EmailList = (props) => {
  ...
  const [filter, setFilter] = useState(
   ...
   const [cancelCampaignConfirmationPopup, setCancelCampaignConfirmationPopup] = useState(false);
    ...
    try {
      ...
      const data = {
        ...
        linksClicked: viewEmailData.click_key
          ? [
              {
                click_name: viewEmailData.click_name || "",
                click_key: viewEmailData.click_key,
              },
  ...
  const handleScheduleEmail = async ({ id, distribute_ids }, index) => {
    try {
      loader("show");
    const {data:response} =  await postData(ENDPOINT.REMOVE_SCHEDULE_EMAIL, {
        campaignId: id,
        distributeIds: distribute_ids,
      });      
      popup_alert({
        visible: "show",
      ...
      const sendListDataTemp = [...SendListData];
      sendListDataTemp[index].status = response?.data?.status;
      setSendListData(sendListDataTemp);
    } catch (error) {
                              ...
                              <ul>
                                ...
                                <li>
                                  <label className="select-multiple-option">
                                    <input
                                      type="checkbox"
                                      id={`custom-checkbox-campaign-8`}
                                      name="campaign[]"
                                      value="Cancelled"
                                      checked={
                                        updateflag > 0 &&
                                        typeof filtercampaign !== "undefined" &&
                                        filtercampaign.indexOf(9) !== -1
                                      }
                                      onChange={() => handleOnFilterCampaign(9)}
                                    />
                                    Cancelled
                                    <span className="checkmark"></span>
                                  </label>
                                  ...
                                  <label className="select-multiple-option">
                                    <input
                                      ...
                                      checked={
                                        updateflag > 0 &&
                                        typeof filtercampaign !== "undefined" &&
                                        filtercampaign.indexOf(5) !== -1
                                      }
                                      onChange={() => handleOnFilterCampaign(5)}
                                    />
                                    Queue
                                    <span className="checkmark"></span>
                                  </label>
                    ...
                    <div className="filter-block-left full">
                      ...
                      {isLikeRdAccount && filtersites.length > 0 && (
                        <div className="filter-div">
                          ...
                          <div className="filter-div-list">
                            {Object.entries(filtersites).map(
                              ([index, item]) => (
                                <div
                                  key={index}
                                  className="filter-result"
              ...
              <div className="col email-result-block">
                ...
                {SendListData.length > 0 ? (
                  SendListData.map((data, index) => {
                    const statusClass = statusClassMap[data?.status] || "draft-approved";
                    const statusLabel = statusLabelMap[data?.status] || "Approved Draft";
                    return (
                      <div className="email_box_block" key={index}>
                      <div className={`email_box ${statusClass}`}>
                          ...
                          <div className="mail-box-content">
                                      ...
                                      {isLikeRdAccount ? (
                                        <>
                                          <tr>
                                            ...
                                            <td>
                                              {data?.unique_site_numbers &&
                                              data?.unique_site_numbers.filter(
                                                (item) => item
                                              ).length === 0
                                                ? "N/A"
                                                : data?.unique_site_numbers
                                ...
                                <ul>
                                  {isRND && (
                                    <li>
                                      ...
                                      <span>
                                        {" "}
                                        {data?.unique_user_types
                                          ? data.unique_user_types.length
                                          : 0}
                                      </span>
                                    </li>
                                  )}
                            ...
                            (
                              <div className="mailbox-buttons">
                                {!deletestatus && data.status != 10 && data?.status != 9 &&(
                                  <div className="mailbox-buttons-list">
            ...
            {typeof viewEmailData !== "undefined" && (
              <div className="modal-body-view">
                <div className="mail-box-content">
                      ...
                      <tbody>
                        ...
                        {isLikeRdAccount ? (
                          <tr>
                            <th>IRTs </th>
                            <td>
                              {viewEmailData[0]?.unique_user_types &&
                              viewEmailData[0]?.unique_user_types.filter(
                                (item) => item
                              ).length > 0
                                ? viewEmailData[0]?.unique_user_types
                                    .filter((item) => item)
                  ...
                  {
                    routeTypeSurvey ?
                    <p><a href={`https://survey.docintel.app/survey_demo?Utmde=`+viewEmailData?.[0]?.surveyUniqueCode} className="doc-link " target="_blank">https://survey.docintel.app/survey_demo?Utmde={viewEmailData?.[0]?.surveyUniqueCode}</a></p>
                    : null
                  }
                  <div className="mail-time">
                    ...
                    >
                      ...
                      <li
                        onClick={() => {
                          getReaderData(
                            "ctr",
                            viewEmailData[0]?.click_name,
                            "#39cabc",
                            viewEmailData[0]?.click_key
                          );
                        }}
                      >
