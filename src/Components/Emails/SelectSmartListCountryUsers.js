...
const SelectSmartListCountryUsers = (props) => {
  ...
  const selectedHcp = location.state ? location.state.selectedHcp : [];
  const removedHcp = location.state ? location.state.removedHcp : [];
    ...
    if (!selectedHcp?.length && !selectedHcp?.length) {
      ...
      axios
        ...
        .then((res) => {
          ...
          let otherCountry = [];
          const processedData = res?.data?.response?.data?.reduce(
            (acc, person) => {
      ...
      selectedHcp?.forEach((person) => {
        const country =
          (person && person.country && person.country.toUpperCase()) || null;
        if (country) {
      ...
      if (removedHcp) {
        removedHcp.forEach((element) => {
          const country =
            (element && element.country && element.country.toUpperCase()) ||
            null;
    ...
    if (countryWiseData.discardCountryData[uppercaseCountry]) {
      ...
      setCountryWiseData((prevData) => ({
        discardCountryData: {
          ...
          [uppercaseCountry]: [
            ...prevData.discardCountryData[uppercaseCountry],
            combinedUserData,
          ],
        },
      ...
      setCountryWiseData((prevData) => ({
        ...
        allCountryData: {
          ...
          [uppercaseCountry]: [
            ...prevData.allCountryData[uppercaseCountry],
            combinedUserData,
          ],
        },
      }));
  ...
  const newlyAddedRemoved = (reader, i, country) => {
    ...
    const newlyAddedReaderList = JSON.parse(
      JSON.stringify(newlyAddedCountryWiseData)
    );
  ...
  const deleteReader = (country, index, i) => {
    ...
    const readersList = JSON.parse(
      JSON.stringify(countryWiseData["allCountryData"])
    );
  ...
  const saveEditClicked = async () => {
    ...
    if (editableData.length > 0) {
      ...
      const userIdsToUpdate = editableData.map(
        (userData) => userData.profile_user_id
      );
      ...
      await axios
        ...
        .then((res) => {
          loader("hide");
          if (res.data.status_code === 200) {
            const updateUserDetails = updateUserByIds(
              userIdsToUpdate,
              editableData
            );
  ...
  const isEmailInBothArrays = (email) => {
    ...
    for (const countryData of Object.values(
      countryWiseData.discardCountryData
    )) {
  ...
  const updateUserByIds = (userIds, updatedDataArray) => {
    ...
    userIds.forEach((userId) => {
      const updatedUserData = updatedDataArray.find(
        (data) => data.profile_user_id === userId
      );

      // Remove user from their current country in discardCountryData
      Object.entries(updatedDiscardCountryData).forEach(([country, users]) => {
        updatedDiscardCountryData[country] = users.filter(
          (user) => user.profile_user_id !== userId
        );
      ...
      Object.entries(updatedAllCountryData).forEach(([country, users]) => {
        updatedAllCountryData[country] = users.filter(
          (user) => user.profile_user_id !== userId
        );
        if (updatedAllCountryData[country] == 0) {
          delete updatedAllCountryData[country];
      ...
      if (updatedDiscardCountryData[targetCountryDiscard]) {
        updatedDiscardCountryData[targetCountryDiscard].push({
          ...updatedUserData,
        ...
        updatedAllCountryData[targetCountryAll].push({
          ...updatedUserData,
          country: updatedUserData.country,
        });
      }
                ...
                <div className="col-12 col-md-9">
                  <ul className="tabnav-link">
                    ...
                    <li className="active active-main">
                      <Link to="/SelectSmartListCountryUsers">
                        Verify Your List
                      </Link>
                    </li>
              ...
              <div className="result-hcp-table">
                ...
                {Object.keys(countryWiseData?.discardCountryData)?.length ? (
                  <Accordion className="deleted">
                    {Object.keys(countryWiseData?.discardCountryData)?.length
                      ? Object.keys(countryWiseData?.discardCountryData)?.map(
                          (country, index) => {
                            return (
                              <>
                                <Accordion.Item eventKey={index}>
                                  <Accordion.Header>
                                    {`${country} (${
                                      ...
                                      (newlyAddedCountryWiseData?.[country]
                                        ?.length
                                        ? newlyAddedCountryWiseData?.[country]
                                            ?.length
                                        : 0)
                ...
                <Accordion>
                  {Object.keys(countryWiseData?.allCountryData)?.length
                    ? Object.keys(countryWiseData?.allCountryData)?.map(
                        (country, index) => {
                          return (
                            <>
                              <Accordion.Item eventKey={index}>
                                ...
                                <Accordion.Body className="card-body">
                                  <div className="selected-hcp-list">
                                    <table className="table">
                                      ...
                                      <tbody>
                                        {countryWiseData?.allCountryData?.[
                                          country
                                        ]?.map((readers, i) => {
                                          return (
                                            <tr
