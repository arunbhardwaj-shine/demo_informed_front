...
const QuestionsPreviewText = () => {
    let params=useParams()
    const [Data, setData] = useState([]);
    const [DataAns, setDataAns] = useState([]);
    const listData=()=>{
        ExportApi.listFormPool_Survey(params.id,"survey",2).then((resp) => {
          if (resp.ok) {
      ...
      const submit=()=>{
       const sub_ques =[]
        ...
        const nodeList1 = document.querySelectorAll("textarea");
    ...
    //   setDataAns([...DataAns]);
     ...
     for (let i = 0; i < nodeList1.length; i++) {
        ...
        let obj={
            id:nodeList1[i].id,
          ques:nodeList1[i].name,
          answer:nodeList1[i].value}
          sub_ques.push(obj)
          console.log(obj);
...
{console.log("first",DataAns)}
 ...
 return   <>
    ...
    {
     item.type=="text"?
     <input id={item.id} name={item.question}/>
   :null}
    {
     item.type=="textarea"?
     <textarea id={item.id} name={item.question}/>
   :null}
...
export default QuestionsPreviewText