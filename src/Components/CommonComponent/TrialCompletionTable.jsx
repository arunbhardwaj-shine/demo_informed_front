...
const TrialCompletionTable = ({
  ...
  const [indidualCompletionTableData, setIndividualCompletionTableData] =
    useState();
  const [
    indidualCompletionTableDataBackup,
    setIndividualCompletionTableDataBackup,
  ] = useState();
  ...
  const applyFilters = () => {
    ...
    Object.keys(appliedFilters).forEach((key) => {
      ...
      if (Array.isArray(value)) {
        if (value.length > 0) keyArr.push(key);
      ...
      <Table className="fold-table" id="individual_completion">
        ...
        <tbody>
          ...
          indidualCompletionTableData?.length > 0 ? (
            <>
              {sortData(indidualCompletionTableData, sortBy, sortOrder)?.map(
                (item, index) => {
                  return (
                    <>
                      <tr
