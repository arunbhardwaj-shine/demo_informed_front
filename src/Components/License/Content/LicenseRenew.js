...
const LicenseRenew = () => {
 ...
 const getLibraryStats = async (event, id) => {
  ...
  const downloadOldCollectedData = () => {
    try {
      if (opening_details?.length == 0) {
        toast.warning("No data found");
        return;
      }

      let downloadData = opening_details?.map((item, index) => {
        return {
                    ...
                    {selectedValue && (
                      ...
                      <div className="renew-license-block">
                        <div className="form-group label">
                            <label>
                              Description dolor sit amet consectetur. Sit euismod convallis eget fermentum. Senectus mattis nibh convallis netus volutpat tempor neque cursus. Et nunc elit tortor leo volutpat.
                            </label>
                        </div>
